(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .mdse-wrapper h3 { margin: 0 0 10px; font-size: 18px; }
      .mdse-wrapper .muted { color: #444; font-size: 13px; font-weight: 700; }
      .mdse-wrapper textarea, .mdse-wrapper input, .mdse-wrapper select {
        border: 2px solid #111; border-radius: 12px; padding: 12px;
        font-size: 16px; line-height: 1.35; background: #fbfbfb; color: #111;
      }
      .mdse-wrapper .mdInput { display: block; width: 100%; max-height: 400px; overflow-y: auto; margin-bottom: 12px; resize: vertical; }
      .mdse-wrapper button {
      ...
      .mdse-wrapper button.warn { border-color: #a00; color: #a00; background: #fffafa; }
      .mdse-wrapper button:hover { opacity: 0.85; }
      .mdse-wrapper button:active { transform: scale(0.96); }
      .mdse-wrapper .btnrow { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; margin: 12px 0; }
      .mdse-wrapper .hdr { display: flex !important; flex-wrap: wrap; align-items: center; gap: 10px; width: 100% !important; margin-bottom: 8px; }
      .mdse-wrapper .title { flex: 1 1 100%; width: 100% !important; display: block; border: 2px solid rgba(0,0,0,.15); border-radius: 12px; padding: 12px 14px; font-size: 17px; font-weight: 800; resize: none; overflow: hidden; background: #fbfbfb; }
      .mdse-wrapper .tabs { display: flex; gap: 10px; flex-wrap: wrap; margin: 6px 0 10px; }
      .mdse-wrapper .tabbtn.active { background: #111; color: #fff; }
      .mdse-wrapper .tabPanel { display: none; width: 100%; }
      .mdse-wrapper .tabPanel.active { display: block; }
      .mdse-wrapper .node { width: 100%; display: block; background: #fff; border: 2px solid rgba(0,0,0,.15); border-radius: 14px; padding: 12px; margin: 10px 0; }
      .mdse-wrapper .node.activeNode { border-color: #0b5fff; box-shadow: 0 8px 22px rgba(11,95,255,.08); }
      .mdse-wrapper .node.activeNode .tools { display: flex !important; }
      .mdse-wrapper .node .tools { display: none; flex-wrap: wrap; gap: 6px; margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee; }
      .mdse-wrapper .body { display: none; margin-top: 10px; }
      .mdse-wrapper .body.show { display: block; }
      .mdse-wrapper .node.movingSource { background: #fff9c4; border: 2px dashed #fbc02d; }
      .mdse-wrapper .node.moveTarget { border: 2px dashed #4caf50; cursor: pointer; }
      .mdse-wrapper .pill { border: 2px solid #111; border-radius: 12px; padding: 4px 8px; font-weight: 900; font-size: 12px; background: #fff; cursor: pointer; }
      .mdse-wrapper .miniTools { display: flex; gap: 4px; margin-left: auto; }
      .mdse-wrapper .miniBtn { border: 1px solid #ddd; padding: 4px 8px; border-radius: 6px; font-size: 12px; background: #fff; cursor: pointer; }
      .mdse-wrapper .miniBtn.primary { background: #0b5fff; color: #fff; border-color: #0b5fff; }
    `;
  }
