(() => {
  ...
  const STYLE_ID = "siteapps-palette-style-v3";
  const BUILD_STAMP = "05 May 2026 06:15 BST · GPT-5.5 Thinking";
  ...
  function ensureStyle() {
    ...
    style.textContent = `
[data-app="palette"]{
  --gap:16px; --radius:14px; --ink:#111; --paper:#fff; --muted:rgba(0,0,0,.68); --line:rgba(0,0,0,.15);
  font-family:system-ui,-apple-system,Segoe UI,Roboto,Inter,Arial,sans-serif;
  max-width:1220px; margin:14px auto; border:2px solid var(--ink); border-radius:16px;
...
[data-app="palette"] .pe-row-2{ display:grid; grid-template-columns:1fr 1fr; gap:10px; }
...
[data-app="palette"] .pe-preview, [data-app="palette"] .pe-grade-preview{ justify-self:center; max-width:100%; }
...
[data-app="palette"] .pe-actions, [data-app="palette"] .pe-harmony-actions{ display:flex; gap:10px; margin-top:10px; flex-wrap:wrap; }
...
[data-app="palette"] textarea:focus, [data-app="palette"] button:focus, [data-app="palette"] input:focus, [data-app="palette"] select:focus{ outline:3px solid rgba(11,95,255,.35); outline-offset:2px; }
@media (min-width: 820px){
  ...
  [data-app="palette"] .pe-grade-grid{ grid-template-columns:1fr 1fr; align-items:start; }
  [data-app="palette"] .pe-grade-picks{ grid-template-columns:1fr 1fr 1fr; }
}
...
`;
  ...
  function labToSrgb(L, a, b) {
    ...
    const invf = (t) => { const t3 = t * t * t; return t3 > 0.008856 ? t3 : (t - 16 / 116) / 7.787; };
    ...
    return [clamp(Math.round(comp(rl) * 255), 0, 255), clamp(Math.round(comp(gl) * 255), 0, 255), clamp(Math.round(comp(bl) * 255), 0, 255)];
  }
  ...
  function kmeansLab(pointsLab, k, maxIter = 20) {
    ...
    for (let iter = 0; iter < maxIter; iter++) {
      ...
      for (let i = 0; i < pointsLab.length; i++) {
        const p = pointsLab[i]; let best = 0, bestD = Infinity;
        for (let c = 0; c < k; c++) {
          const cc = centroids[c]; const dx = p[0] - cc[0], dy = p[1] - cc[1], dz = p[2] - cc[2]; const ds = dx * dx + dy * dy + dz * dz;
          if (ds < bestD) { bestD = ds; best = c; }
        }
        if (assignments[i] !== best) { assignments[i] = best; moved++; }
      }
      const sums = Array.from({ length: k }, () => [0, 0, 0, 0]);
      for (let i = 0; i < pointsLab.length; i++) { const g = assignments[i], p = pointsLab[i]; sums[g][0] += p[0]; sums[g][1] += p[1]; sums[g][2] += p[2]; sums[g][3]++; }
      let drift = 0;
      for (let c = 0; c < k; c++) {
        ...
        drift += Math.sqrt(dx * dx + dy * dy + dz * dz); centroids[c] = nc;
      }
  ...
  function pixelsFromImageData(imgData, sampleStep = 4, includeExtremes = false) {
    ...
    for (let i = 0; i < data.length; i += 4 * sampleStep) {
      ...
      if (!includeExtremes) { const lum = relativeLuminance(r, g, b); if (lum > 0.97 || lum < 0.06) continue; }
  ...
  async function copyText(text) {
    try { await navigator.clipboard.writeText(text); return true; }
      ...
      try { const ta = document.createElement("textarea"); ta.value = text; ta.style.position = "fixed"; ta.style.left = "-9999px"; document.body.appendChild(ta); ta.focus(); ta.select(); const ok = document.execCommand("copy"); ta.remove(); return !!ok; }
      catch { return false; }
    }
  }

  const HARMONY_METHODS = {
    complementary: { label: "Complementary", note: "One colour opposite the base hue. Useful for strong subject/background separation or a simple colour-grade tension.", offsets: [0, 180] },
    split: { label: "Split complementary", note: "The base colour plus two colours either side of the direct complement. Often easier to use than a hard complement.", offsets: [0, 150, 210] },
    triadic: { label: "Triadic", note: "Three hues evenly spaced around the wheel. Energetic, graphic, and easy to overdo in natural photos.", offsets: [0, 120, 240] },
    tetradic: { label: "Tetradic / rectangle", note: "Two complementary pairs. Useful for exploring a richer grade, but usually one colour should dominate.", offsets: [0, 60, 180, 240] },
    analogous: { label: "Analogous", note: "Nearby hues. Better for gentle, cohesive palettes where you do not want a dramatic opposite colour.", offsets: [-30, 0, 30] },
    monochrome: { label: "Monochrome tonal set", note: "Same hue, varied lightness and saturation. Good for checking whether a grade is relying on tone rather than hue contrast.", offsets: [0, 0, 0, 0] },
    square: { label: "Square", note: "Four colours spaced 90° apart. Strong and decorative; for photography, treat this as a scouting tool rather than a rule.", offsets: [0, 90, 180, 270] }
  };
  ...
  function buildHarmony(baseHsl, methodKey, satAdjust = 0, lightAdjust = 0) {
    ...
    const s = clamp(baseS + satAdjust, 5, 100), l = clamp(baseL + lightAdjust, 8, 92);
    return method.offsets.map((off, i) => {
      let h = mod(baseH + off, 360), ss = s, ll = l, role = i === 0 ? "Base" : (off === 180 ? "Complement" : `${off > 0 ? "+" : ""}${off}°`);
  ...
  function overlayBlendChannel(base, blend) { base /= 255; blend /= 255; const out = base < 0.5 ? 2 * base * blend : 1 - 2 * (1 - base) * (1 - blend); return clamp(Math.round(out * 255), 0, 255); }
  ...
  function applyGradeToImageData(imgData, opts) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      const a = d[i + 3]; if (a < 1) continue;
      let r = d[i], g = d[i + 1], b = d[i + 2];
      const lum = relativeLuminance(r, g, b);

      let sw = 1 - smoothstep(0.12, 0.58, lum);
      let hw = smoothstep(0.42, 0.94, lum);
      let mw = 1 - Math.abs(lum - 0.5) * 2;
      mw = clamp(mw, 0, 1);

      if (protect > 0) {
        const whiteGuard = smoothstep(0.78, 1.0, lum) * protect;
        sw *= (1 - whiteGuard); mw *= (1 - whiteGuard * 0.7); hw *= (1 - whiteGuard * 0.35);
      }

      const sum = Math.max(0.0001, sw + mw + hw);
      sw /= sum; mw /= sum; hw /= sum;
      const tint = [
        sh[0] * sw + mid[0] * mw + hi[0] * hw,
        sh[1] * sw + mid[1] * mw + hi[1] * hw,
        sh[2] * sw + mid[2] * mw + hi[2] * hw
      ];

      const or = overlayBlendChannel(r, tint[0]);
      const og = overlayBlendChannel(g, tint[1]);
      const ob = overlayBlendChannel(b, tint[2]);
      r = lerp(r, or, strength); g = lerp(g, og, strength); b = lerp(b, ob, strength);

      if (contrast !== 0) {
        const c = contrast * 1.15;
        r = (r - 128) * (1 + c) + 128; g = (g - 128) * (1 + c) + 128; b = (b - 128) * (1 + c) + 128;
      }

      if (satBoost !== 0) {
        const grey = 0.2126 * r + 0.7152 * g + 0.0722 * b;
        const factor = 1 + satBoost;
        r = grey + (r - grey) * factor; g = grey + (g - grey) * factor; b = grey + (b - grey) * factor;
      }
...
<div class="pe-app">
  ...
  <div class="pe-actions">
    ...
    <button class="pe-btn pe-dl-png" disabled>Download palette PNG</button>
    <button class="pe-btn pe-dl-svg" disabled>Download palette SVG</button>
  ...
  <div class="pe-grid-2">
    <section class="pe-panel pe-wheel-panel" aria-label="Colour wheel and harmony controls">
      ...
      <div class="pe-wheel-layout">
        ...
        <div class="pe-wheel-form">
          ...
          <div class="pe-harmony-actions"><button class="pe-btn pe-copy-harmony-hex">Copy harmony HEX</button><button class="pe-btn pe-copy-harmony-css">Copy harmony CSS</button><button class="pe-btn pe-copy-harmony-tsv">Copy harmony table</button></div>
        </div>
      </div>
    </section>
    ...
    <section class="pe-panel" aria-label="Suggested colour grade notes">
      ...
      <div class="pe-small"><strong>Practical rule:</strong> pick one dominant colour direction, one support colour, and one accent. The grade preview is intentionally restrained so you can test a mood before moving to Lightroom.</div>
  ...
  <section class="pe-panel pe-grade-grid" aria-label="Grade preview" style="margin-top:18px;">
    <div>
      <h3 class="pe-panel-title">Grade preview</h3>
      <canvas class="pe-grade-canvas" width="0" height="0" aria-label="Graded image preview"></canvas>
      <div class="pe-small" style="margin-top:8px;">Preview uses a working canvas for speed. Export uses the loaded image dimensions where possible.</div>
    </div>
    <div class="pe-grade-form">
      <div class="pe-grade-picks">
        <label><strong>Shadows</strong><input class="pe-shadow-color" type="color" value="#163A5F"></label>
        <label><strong>Midtones</strong><input class="pe-mid-color" type="color" value="#6F6A60"></label>
        <label><strong>Highlights</strong><input class="pe-highlight-color" type="color" value="#D89A4A"></label>
      </div>
      ...
      <div class="pe-actions">
        <button class="pe-btn pe-primary pe-apply-grade" disabled>Apply preview grade</button>
        <button class="pe-btn pe-use-harmony-grade">Use harmony as grade</button>
        <button class="pe-btn pe-grade-warm">Warm highlights / cool shadows</button>
        <button class="pe-btn pe-grade-cool">Cool muted grade</button>
        <button class="pe-btn pe-grade-reset">Reset grade</button>
        <button class="pe-btn pe-export-grade" disabled>Export graded PNG</button>
      </div>
    </div>
  </section>
  ...
  <canvas class="pe-full-canvas" width="0" height="0" style="display:none;"></canvas>
  ...
  <div class="pe-meta">Build ${BUILD_STAMP}</div>
    ...
    function toast(msg) { toastEl.textContent = msg; toastEl.classList.add("show"); clearTimeout(toast._t); toast._t = setTimeout(() => toastEl.classList.remove("show"), 1400); }
    function enableActions(enabled) { [extractBtn, copyHexBtn, copyCssBtn, copyTsvBtn, dlPngBtn, dlSvgBtn, dlJsonBtn, dlCsvBtn, dlCssBtn, dlGplBtn].forEach((b) => b.disabled = !enabled); applyGradeBtn.disabled = !enabled; exportGradeBtn.disabled = !enabled; }
    ...
    async function loadFile(file) {
      ...
      imgEl.style.display = "none"; imgEl.src = lastObjectUrl;
      ...
      canvas.width = Math.max(1, Math.floor(w * previewScale)); canvas.height = Math.max(1, Math.floor(h * previewScale));
      canvas.getContext("2d", { willReadFrequently: true }).drawImage(imgEl, 0, 0, canvas.width, canvas.height);
      fullCanvas.width = w; fullCanvas.height = h; fullCanvas.getContext("2d", { willReadFrequently: true }).drawImage(imgEl, 0, 0, w, h);
      imgEl.style.display = "block"; enableActions(true); applyGradePreview(); toast("Image ready");
    }
    ...
    function paletteToTableRows(palette) {
      return palette.map((c, i) => {
        const rgb = `rgb(${c.rgb[0]}, ${c.rgb[1]}, ${c.rgb[2]})`, hsl = `hsl(${c.hsl[0]} ${c.hsl[1]}% ${c.hsl[2]}%)`, role = c.role ? `<td>${c.role}</td>` : "";
        return `<tr><td>${String(i + 1).padStart(2, "0")}</td><td><span class="pe-sample" style="background:${c.hex}"></span></td><td>${c.hex}</td><td>${rgb}</td><td>${hsl}</td><td>${c.name}</td>${role}</tr>`;
      }).join("");
    }
    function renderListTable(palette, targetEl, includeRole = false) { targetEl.innerHTML = `<div class="pe-list"><table><thead><tr><th>#</th><th>Sample</th><th>HEX</th><th>RGB</th><th>HSL</th><th>CSS Var</th>${includeRole ? "<th>Role</th>" : ""}</tr></thead><tbody>${paletteToTableRows(palette)}</tbody></table></div>`; }
    function setBaseColour(hex, silent = false) { const rgb = hexToRgb(hex); if (!rgb) { if (!silent) toast("Invalid HEX colour"); return; } const cleanHex = rgbToHex(rgb[0], rgb[1], rgb[2]); baseColorInput.value = cleanHex; baseHexInput.value = cleanHex; renderHarmony(); }

    function makeSwatchEl({ hex, rgb, hsl, role }, opts = {}) {
      const [r, g, b] = rgb, [h, s, l] = hsl, text = bestTextColor(r, g, b);
      const el = document.createElement("div"); el.className = "pe-swatch";
      const chip = document.createElement("div"); chip.className = "pe-chip"; chip.style.background = hex; chip.style.color = text; chip.title = `${hex} · hsl(${h} ${s}% ${l}%)`; chip.innerHTML = `<span>${role || hex}</span>`;
      chip.addEventListener("click", () => { setBaseColour(hex); toast("Base colour set from swatch"); });
      const info = document.createElement("div"); info.className = "pe-info";
      const line = (label, value) => { const row = document.createElement("div"); row.className = "pe-line"; const tag = document.createElement("div"); tag.className = "pe-tag"; tag.textContent = value; const btn = document.createElement("button"); btn.className = "pe-copy"; btn.type = "button"; btn.textContent = "Copy"; btn.addEventListener("click", async (e) => { e.stopPropagation(); toast((await copyText(value)) ? `Copied ${label}` : "Copy failed"); }); row.append(tag, btn); return row; };
      if (role) info.append(line("role", role)); info.append(line("HEX", hex), line("RGB", `rgb(${r}, ${g}, ${b})`), line("HSL", `hsl(${h} ${s}% ${l}%)`));
      if (opts.showSetBase !== false) { const btn = document.createElement("button"); btn.className = "pe-copy"; btn.type = "button"; btn.textContent = "Use as base"; btn.addEventListener("click", (e) => { e.stopPropagation(); setBaseColour(hex); toast("Base colour set"); }); info.append(btn); }
      el.append(chip, info); return el;
    }

    function drawWheel() {
      const ctx = wheelCanvas.getContext("2d", { willReadFrequently: true }), w = wheelCanvas.width, h = wheelCanvas.height, cx = w / 2, cy = h / 2, radius = Math.min(w, h) / 2 - 4, img = ctx.createImageData(w, h);
      for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
        const dx = x - cx, dy = y - cy, d = Math.sqrt(dx * dx + dy * dy), idx = (y * w + x) * 4;
        if (d <= radius) { const angle = mod(Math.atan2(dy, dx) * 180 / Math.PI + 360, 360), sat = clamp((d / radius) * 100, 0, 100), [r, g, b] = hslToRgb(angle, sat, 50); img.data[idx] = r; img.data[idx + 1] = g; img.data[idx + 2] = b; img.data[idx + 3] = 255; }
        else { img.data[idx + 3] = 0; }
      }
      ctx.putImageData(img, 0, 0);
    }
    function markerPositionForHsl(h, s) { const rect = wheelCanvas.getBoundingClientRect(), cx = rect.width / 2, cy = rect.height / 2, radius = rect.width / 2 - 4, a = mod(h, 360) * Math.PI / 180, d = clamp(s, 0, 100) / 100 * radius; return { x: cx + Math.cos(a) * d, y: cy + Math.sin(a) * d }; }
    function placeBaseMarker() { const rgb = hexToRgb(baseHexInput.value) || [51, 102, 204], [h, s] = rgbToHsl(rgb[0], rgb[1], rgb[2]), p = markerPositionForHsl(h, s); baseMarker.style.left = `${p.x}px`; baseMarker.style.top = `${p.y}px`; const method = HARMONY_METHODS[methodSelect.value] || HARMONY_METHODS.complementary; wheelBox.querySelectorAll(".pe-related").forEach((m) => m.remove()); method.offsets.slice(1).forEach((off) => { const pp = markerPositionForHsl(h + off, s), m = document.createElement("div"); m.className = "pe-wheel-marker pe-related"; m.style.left = `${pp.x}px`; m.style.top = `${pp.y}px`; wheelBox.appendChild(m); }); }
    function wheelEventToColour(e) { const rect = wheelCanvas.getBoundingClientRect(), x = e.clientX - rect.left, y = e.clientY - rect.top, cx = rect.width / 2, cy = rect.height / 2, dx = x - cx, dy = y - cy, d = Math.sqrt(dx * dx + dy * dy), radius = rect.width / 2 - 4; if (d > radius) return null; const angle = mod(Math.atan2(dy, dx) * 180 / Math.PI + 360, 360), sat = clamp((d / radius) * 100, 0, 100), current = hexToRgb(baseHexInput.value) || [51, 102, 204], currentHsl = rgbToHsl(current[0], current[1], current[2]); return hslToHex(angle, sat, currentHsl[2]); }

    function renderHarmony() {
      const rgb = hexToRgb(baseHexInput.value) || [51, 102, 204], baseHsl = rgbToHsl(rgb[0], rgb[1], rgb[2]), methodKey = methodSelect.value, method = HARMONY_METHODS[methodKey] || HARMONY_METHODS.complementary;
      const satDelta = parseInt(satAdjust.value, 10) || 0, lightDelta = parseInt(lightAdjust.value, 10) || 0; satLabel.textContent = String(satDelta); lightLabel.textContent = String(lightDelta);
      lastHarmony = buildHarmony(baseHsl, methodKey, satDelta, lightDelta); harmonyPaletteEl.innerHTML = ""; lastHarmony.forEach((c) => harmonyPaletteEl.appendChild(makeSwatchEl(c, { showSetBase: true }))); renderListTable(lastHarmony, harmonyListEl, true);
      methodNote.textContent = method.note; angleList.innerHTML = method.offsets.map((o, i) => { const angle = mod(baseHsl[0] + o, 360), label = methodKey === "monochrome" ? lastHarmony[i].role : `${lastHarmony[i].role}: ${Math.round(angle)}°`; return `<span class="pe-angle-pill">${label}</span>`; }).join("");
      placeBaseMarker();
    }

    wheelCanvas.addEventListener("click", (e) => { const hex = wheelEventToColour(e); if (!hex) return; setBaseColour(hex); toast("Base colour picked"); });
...
")}
    ...
    function harmonyTsvText() { const header = ["index", "role", "hex", "rgb", "hsl", "css_var"].join("	"); const lines = lastHarmony.map((c, i) => [i + 1, c.role, c.hex, `rgb(${c.rgb[0]}, ${c.rgb[1]}, ${c.rgb[2]})`, `hsl(${c.hsl[0]} ${c.hsl[1]}% ${c.hsl[2]}%)`, c.name].join("	")); return [header, ...lines].join("
"); }
    ...
    function renderQuickPicks(picks) { quickPicks.innerHTML = ""; picks.forEach((hex) => { const b = document.createElement("button"); b.type = "button"; b.className = "pe-quick-pick"; b.style.background = hex; b.title = `Use ${hex} as base`; b.addEventListener("click", () => setBaseColour(hex)); quickPicks.appendChild(b); }); }

    function getGradeOptions() { return { shadowRgb: hexToRgb(shadowColor.value) || [22, 58, 95], midRgb: hexToRgb(midColor.value) || [111, 106, 96], highlightRgb: hexToRgb(highlightColor.value) || [216, 154, 74], strength: parseInt(gradeStrength.value, 10) || 0, contrast: parseInt(gradeContrast.value, 10) || 0, saturation: parseInt(gradeSaturation.value, 10) || 0, protectWhites: parseInt(protectWhites.value, 10) || 0 }; }
    function updateGradeLabels() { gradeStrengthLabel.textContent = gradeStrength.value; gradeContrastLabel.textContent = gradeContrast.value; gradeSaturationLabel.textContent = gradeSaturation.value; protectWhitesLabel.textContent = protectWhites.value; }
    function applyGradePreview() {
      updateGradeLabels();
      if (canvas.width === 0 || canvas.height === 0) return;
      gradeCanvas.width = canvas.width; gradeCanvas.height = canvas.height; gradeCanvas.style.display = "block";
      const ctx = gradeCanvas.getContext("2d", { willReadFrequently: true }); ctx.drawImage(canvas, 0, 0);
      const imgData = ctx.getImageData(0, 0, gradeCanvas.width, gradeCanvas.height); ctx.putImageData(applyGradeToImageData(imgData, getGradeOptions()), 0, 0);
      toast("Grade preview applied");
    }
    function useHarmonyAsGrade() {
      if (!lastHarmony.length) renderHarmony();
      const hs = lastHarmony;
      shadowColor.value = hs[1]?.hex || hs[0]?.hex || "#163A5F";
      midColor.value = hs[0]?.hex || "#6F6A60";
      highlightColor.value = hs[2]?.hex || hs[1]?.hex || "#D89A4A";
      gradeStrength.value = "22"; gradeContrast.value = "6"; gradeSaturation.value = "0"; protectWhites.value = "60";
      applyGradePreview();
    }
    applyGradeBtn.addEventListener("click", applyGradePreview);
    [shadowColor, midColor, highlightColor, gradeStrength, gradeContrast, gradeSaturation, protectWhites].forEach((el) => el.addEventListener("input", () => { updateGradeLabels(); clearTimeout(applyGradePreview._t); applyGradePreview._t = setTimeout(applyGradePreview, 160); }));
    ...
    exportGradeBtn.addEventListener("click", () => {
      ...
      const ctx = fullCanvas.getContext("2d", { willReadFrequently: true }); ctx.drawImage(imgEl, 0, 0, fullCanvas.width, fullCanvas.height);
      const imgData = ctx.getImageData(0, 0, fullCanvas.width, fullCanvas.height); ctx.putImageData(applyGradeToImageData(imgData, getGradeOptions()), 0, 0);
      const a = document.createElement("a"); a.download = "graded-image.png"; a.href = fullCanvas.toDataURL("image/png"); a.click(); toast("Exported graded PNG");
    });

    async function extract() {
      if (canvas.width === 0 || canvas.height === 0) return;
      const ctx = canvas.getContext("2d", { willReadFrequently: true }), imgData = ctx.getImageData(0, 0, canvas.width, canvas.height), lab = pixelsFromImageData(imgData, 4, includeExt.checked);
      if (lab.length < 8) { toast("Not enough pixels to sample"); return; }
      const k = parseInt(countRange.value, 10), { centroids } = kmeansLab(lab, k, 20);
      const colours = centroids.map((c) => { const [r, g, b] = labToSrgb(c[0], c[1], c[2]), [h, s, l] = rgbToHsl(r, g, b); return { r, g, b, h, s, l, hex: rgbToHex(r, g, b) }; }).sort((a, b) => (a.l - b.l) || (a.h - b.h));
      paletteEl.innerHTML = ""; lastPalette = colours.map((c, i) => { const item = { hex: c.hex, rgb: [c.r, c.g, c.b], hsl: [c.h, c.s, c.l], name: `--color-${String(i + 1).padStart(2, "0")}` }; paletteEl.appendChild(makeSwatchEl(item, { showSetBase: true })); return item; });
      renderListTable(lastPalette, listWrap, false); renderQuickPicks(lastPalette.map((c) => c.hex));
      if (lastPalette.length) { const mid = lastPalette[Math.min(lastPalette.length - 1, Math.max(0, Math.floor(lastPalette.length / 2)))]; setBaseColour(mid.hex, true); }
...
")}
      ...
      copyTsvBtn.onclick = async () => { const header = ["index", "hex", "rgb", "hsl", "css_var"].join("	"), lines = lastPalette.map((c, i) => [i + 1, c.hex, `rgb(${c.rgb[0]}, ${c.rgb[1]}, ${c.rgb[2]})`, `hsl(${c.hsl[0]} ${c.hsl[1]}% ${c.hsl[2]}%)`, c.name].join("	")); toast((await copyText([header, ...lines].join("
"))) ? "Table copied (TSV)" : "Copy failed"); };
      ...
      dlCsvBtn.onclick = () => { const esc = (v) => `"${String(v).replace(/"/g, '""')}"`, header = ["set", "index", "role", "hex", "r", "g", "b", "h", "s", "l", "css_var"], rows = [...lastPalette.map((c, i) => ["extracted", i + 1, "", c.hex, c.rgb[0], c.rgb[1], c.rgb[2], c.hsl[0], c.hsl[1], c.hsl[2], c.name]), ...lastHarmony.map((c, i) => ["harmony", i + 1, c.role, c.hex, c.rgb[0], c.rgb[1], c.rgb[2], c.hsl[0], c.hsl[1], c.hsl[2], c.name])]; download("palette.csv", [header, ...rows].map((r) => r.map(esc).join(",")).join("
"), "text/csv"); toast("Downloaded CSV"); };
...
`, "text/css"); toast("Downloaded CSS"); };
      dlPngBtn.onclick = () => { const swatches = [...lastPalette, ...lastHarmony], swatchW = 140, height = 180, width = Math.max(1, swatches.length) * swatchW; dlCanvas.width = width; dlCanvas.height = height; const c = dlCanvas.getContext("2d"); c.fillStyle = "#fff"; c.fillRect(0, 0, width, height); c.textAlign = "center"; c.textBaseline = "middle"; c.font = "15px system-ui"; swatches.forEach((item, i) => { const x = i * swatchW; c.fillStyle = item.hex; c.fillRect(x, 0, swatchW, height - 54); c.fillStyle = "#000"; c.fillRect(x, height - 54, swatchW, 54); c.fillStyle = "#fff"; c.fillText(item.hex, x + swatchW / 2, height - 34); c.font = "12px system-ui"; c.fillText(item.role || "Extracted", x + swatchW / 2, height - 15); c.font = "15px system-ui"; }); const a = document.createElement("a"); a.download = "palette.png"; a.href = dlCanvas.toDataURL("image/png"); a.click(); toast("Downloaded PNG"); };
      dlSvgBtn.onclick = () => { const swatches = [...lastPalette, ...lastHarmony], sw = 170, h = 210, w = Math.max(1, swatches.length) * sw, blocks = swatches.map((c, i) => `<g><rect x="${i * sw}" y="0" width="${sw}" height="${h - 58}" fill="${c.hex}"/><rect x="${i * sw}" y="${h - 58}" width="${sw}" height="58" fill="#000"/><text x="${i * sw + sw / 2}" y="${h - 34}" text-anchor="middle" font-family="system-ui,-apple-system,Segoe UI,Roboto" font-size="16" fill="#fff">${c.hex}</text><text x="${i * sw + sw / 2}" y="${h - 14}" text-anchor="middle" font-family="system-ui,-apple-system,Segoe UI,Roboto" font-size="12" fill="#fff">${c.role || "Extracted"}</text></g>`).join(""); download("palette.svg", `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><rect width="100%" height="100%" fill="#fff"/>${blocks}</svg>`, "image/svg+xml"); toast("Downloaded SVG"); };
      dlGplBtn.onclick = () => { const head = `GIMP Palette
Name: Extracted + Harmony Palette
Columns: ${Math.max(lastPalette.length, lastHarmony.length)}
#`, lines = [...lastPalette, ...lastHarmony].map((c) => `${String(c.rgb[0]).padStart(3, " ")} ${String(c.rgb[1]).padStart(3, " ")} ${String(c.rgb[2]).padStart(3, " ")}	${c.role || c.hex} ${c.hex}`); download("palette.gpl", [head, ...lines].join("
"), "text/plain"); toast("Downloaded .gpl"); };
      useHarmonyAsGrade(); toast(`Found ${lastPalette.length} colours`);
    }

    extractBtn.addEventListener("click", extract);
    enableActions(false); drawWheel(); renderQuickPicks(["#1E3A5F", "#2F6F4E", "#6B5B95", "#A64B2A", "#D98C00", "#3B6EA8", "#7B2D26", "#B07D62", "#708238", "#4A4A4A"]); renderHarmony(); updateGradeLabels(); window.addEventListener("resize", placeBaseMarker); toast("Ready");
  });
})();
