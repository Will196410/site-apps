(() => {
  ...
  const PAST_VERBS = new Set([
    "was", "were", "had", "did", "said", "went", "made", "took", "saw",
    "felt", "thought", "knew", "came", "gave", "got", "found", "told",
    "asked", "wanted", "needed", "tried", "turned", "moved", "stood",
    "sat", "walked", "ran", "held", "kept", "left", "heard", "watched",
    "pulled", "pushed", "opened", "closed", "killed", "lived", "died",
    "became", "began", "broke", "brought", "built", "caught", "chose",
    "cut", "drew", "drove", "fell", "fought", "grew", "hid", "lost",
    "met", "paid", "put", "read", "rose", "sent", "slept", "spoke",
    "struck", "swore", "threw", "wore", "won", "wrote"
  ]);

  const PRESENT_AUX = new Set(["am", "are", "is", "do", "does", "have", "has"]);
  const PAST_AUX = new Set(["was", "were", "did", "had"]);
  const FUTURE_MARKERS = new Set(["will", "shall", "won't", "shan't"]);
  const MODAL_MARKERS = new Set(["would", "could", "should", "might", "may", "must", "can"]);
  const PERFECT_PARTICIPLES = new Set([
    "been", "gone", "seen", "known", "given", "taken", "made", "found",
    "told", "left", "heard", "held", "kept", "felt", "thought", "said",
    "done", "come", "become", "written", "spoken", "broken", "chosen",
    "fallen", "forgotten", "hidden", "lost", "met", "sent", "slept", "worn", "won"
  ]);
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      [data-app="analyzer"] * { box-sizing: border-box; }
      ...
      [data-app="analyzer"] #input-text { width: 100%; min-height: 500px; border: none; padding: 20px; font-family: ui-monospace, monospace; font-size: 16px; line-height: 1.6; outline: none; display: block; }
      [data-app="analyzer"] .control-stack { display: flex; flex-direction: column; gap: 20px; margin-bottom: 20px; width: 100%; }
      [data-app="analyzer"] .panel { background: #f9f9f9; padding: 15px; border: 2px solid #000; border-radius: 4px; width: 100%; min-width: 0; }
      [data-app="analyzer"] .panel h3 { margin-top: 0; }
      [data-app="analyzer"] .switch-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 16px; }
      [data-app="analyzer"] .switch-item { display: flex; align-items: center; gap: 10px; font-weight: 900; text-transform: uppercase; font-size: 12px; margin-bottom: 8px; cursor: pointer; line-height: 1.4; }
      [data-app="analyzer"] .switch-item input { flex: 0 0 auto; }
      [data-app="analyzer"] .filter-input-group { display: flex; gap: 5px; margin-bottom: 10px; width: 100%; }
      [data-app="analyzer"] .filter-input-group input { flex: 1 1 auto; min-width: 0; padding: 8px; border: 2px solid #000; outline: none; }
      [data-app="analyzer"] .btn-small { background: #000; color: #fff; border: none; padding: 8px 12px; cursor: pointer; font-weight: bold; text-transform: uppercase; font-size: 11px; white-space: nowrap; }
      [data-app="analyzer"] .btn-small:hover { background: #ffff00; color: #000; }
      [data-app="analyzer"] .filter-table-wrap { max-height: 200px; overflow-y: auto; overflow-x: auto; border: 1px solid #ddd; background: #fff; width: 100%; }
      ...
      [data-app="analyzer"] td { padding: 6px 8px; border-bottom: 1px solid #eee; word-break: break-word; }
      ...
      [data-app="analyzer"] .pos-banner { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 20px; font-size: 12px; font-weight: 900; }
      [data-app="analyzer"] .pos-pill { border: 2px solid #000; padding: 6px 8px; background: #fff; }
      [data-app="analyzer"] .line-container { display: flex; border-bottom: 1px solid #f0f0f0; }
      [data-app="analyzer"] .line-num { width: 45px; color: #aaa; text-align: right; padding-right: 12px; border-right: 1px solid #eee; margin-right: 12px; font-family: monospace; user-select: none; flex: 0 0 auto; }
      [data-app="analyzer"] .line-txt { flex: 1; min-width: 0; white-space: pre-wrap; padding: 4px 0; font-family: ui-monospace, monospace; line-height: 1.6; overflow-wrap: anywhere; }
      [data-app="analyzer"] mark.hl { background: #000; color: #ffff00; padding: 0 2px; font-weight: bold; }
      [data-app="analyzer"] mark.hl-phrase { background: #000; color: #ffff00; padding: 0 2px; font-weight: bold; box-shadow: inset 0 -3px 0 #ffff00; }
      [data-app="analyzer"] mark.pos { background: #fff; color: #000; padding: 0 2px; border: 2px solid #000; font-weight: bold; }
      [data-app="analyzer"] mark.pos-adverb { border-style: dashed; }
      [data-app="analyzer"] mark.pos-verb { border-style: solid; }
      [data-app="analyzer"] mark.pos-noun { border-style: double; }
      [data-app="analyzer"] mark.pos-adjective { border-style: dotted; }
      [data-app="analyzer"] mark.tense { background: #ffff00; color: #000; padding: 0 2px; border: 3px solid #000; font-weight: bold; }
      [data-app="analyzer"] mark.tense-past { box-shadow: inset 0 -4px 0 #000; }
      [data-app="analyzer"] mark.tense-future { outline: 3px dashed #000; outline-offset: 1px; }
      [data-app="analyzer"] mark.tense-modal, [data-app="analyzer"] mark.tense-perfect, [data-app="analyzer"] mark.tense-unknown { outline: 2px dotted #000; outline-offset: 1px; }
      [data-app="analyzer"] .summary-box { background: #000; color: #ffff00; padding: 15px; margin-bottom: 20px; font-family: monospace; border-left: 10px solid #ffff00; overflow-wrap: anywhere; }

      @media (max-width: 640px) {
        [data-app="analyzer"] { padding: 12px; }
        [data-app="analyzer"] #input-text { min-height: 360px; padding: 14px; font-size: 15px; }
        [data-app="analyzer"] .switch-grid { grid-template-columns: 1fr; }
        [data-app="analyzer"] .filter-input-group { flex-direction: column; }
        [data-app="analyzer"] .filter-input-group input, [data-app="analyzer"] .filter-input-group button { width: 100%; }
        [data-app="analyzer"] .btn-main { font-size: 16px; padding: 15px; }
        [data-app="analyzer"] .stats-banner { gap: 10px; font-size: 12px; }
        [data-app="analyzer"] .line-num { width: 36px; padding-right: 8px; margin-right: 8px; }
      }
    `;
  ...
  function parseProse(text) {
    return text.split("\n").map((lineText, index) => {
      ...
      return { lineNumber, text: lineText, tokens, wordTokens, sentences };
    });
  }
  ...
  function compileFilter(filter) {
    ...
    return { raw, normalized, parts, length: parts.length };
  }
  ...
  function classifyPOS(word, prevWord, nextWord) {
    ...
    if (clean.endsWith("ly") && !LY_EXCEPTIONS.has(clean)) return "adverb";

    if (COMMON_VERBS.has(clean) || clean.endsWith("ing") || clean.endsWith("ed") || (clean.endsWith("en") && clean.length > 4)) return "verb";

    if (
      COMMON_ADJECTIVES.has(clean) || clean.endsWith("ous") || clean.endsWith("ful") ||
      clean.endsWith("less") || clean.endsWith("able") || clean.endsWith("ible") ||
      clean.endsWith("al") || clean.endsWith("ive") || clean.endsWith("ic") ||
      clean.endsWith("ish") || clean.endsWith("ary")
    ) return "adjective";

    if (
      COMMON_NOUNS.has(clean) || clean.endsWith("tion") || clean.endsWith("sion") ||
      clean.endsWith("ment") || clean.endsWith("ness") || clean.endsWith("ity") ||
      clean.endsWith("ship") || clean.endsWith("hood") || clean.endsWith("ance") ||
      clean.endsWith("ence")
    ) return "noun";

    if (DETERMINERS.has(prev)) return "noun";
    if (PREPOSITIONS.has(prev) && !COMMON_VERBS.has(clean)) return "noun";
    if (next && !DETERMINERS.has(clean) && !PREPOSITIONS.has(clean) && COMMON_NOUNS.has(next)) return "adjective";
    if (clean.endsWith("s") && clean.length > 3 && !clean.endsWith("ss")) return "noun";
  ...
  function classifySentenceTense(sentence) {
    const words = sentence.tokens
      ...
      .map(t => ({ text: t.text, clean: normalizeWord(t.text), start: t.start, end: t.end }));

    if (!words.length) return { tense: "unknown", label: "Unknown", reason: "No words found", severity: 0 };
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (PRESENT_AUX.has(clean)) hasPresent = true;
      ...
      if (looksPastSimple(clean)) {
        const followsPresentPerfect = (prev === "has" || prev === "have") && looksPastParticiple(clean);
      ...
      if (clean.endsWith("s") && clean.length > 3 && !clean.endsWith("ss") && !COMMON_NOUNS.has(clean)) {
        hasPresent = true;
      }
    }

    if (hasFuture) return { tense: "future", label: "Future / Non-present", reason: `Future marker: ${evidence.slice(0, 3).join(", ")}`, severity: 3 };
    if (hasPast) return { tense: "past", label: "Past / Non-present", reason: `Past-tense evidence: ${evidence.slice(0, 3).join(", ")}`, severity: 3 };
    if (hasModal) return { tense: "modal", label: "Modal / Conditional", reason: `Modal marker: ${evidence.slice(0, 3).join(", ")}`, severity: 2 };
    if (hasPerfect) return { tense: "perfect", label: "Perfect aspect", reason: `Perfect construction: ${evidence.slice(0, 3).join(", ")}`, severity: 2 };
    if (hasPresent || hasProgressive) return { tense: "present", label: "Present", reason: "Likely present tense", severity: 0 };

    return { tense: "unknown", label: "Unclear tense", reason: "No clear finite verb detected", severity: 1 };
  }
  ...
  function renderLineWithSpans(lineText, spans) {
    ...
    spans.forEach(span => {
      if (span.start > pos) output += escapeHTML(lineText.slice(pos, span.start));
    ...
    if (pos < lineText.length) output += escapeHTML(lineText.slice(pos));
    return output;
  }
  ...
  window.SiteApps.register("analyzer", (container) => {
      ...
      <div class="control-stack">
        <div class="panel">
          ...
          <div class="switch-grid">
            ...
            <label class="switch-item"><input type="checkbox" id="sw-tenseCheck"> Flag Non-Present Tense</label>
            ...
            <label class="switch-item"><input type="checkbox" id="sw-rememberText"> Remember Text On This Device</label>
          </div>

          <h3 style="margin-top:18px;">Parts of Speech</h3>
          <div class="switch-grid">
            <label class="switch-item"><input type="checkbox" id="sw-posAdverbs"> Highlight Adverbs</label>
            <label class="switch-item"><input type="checkbox" id="sw-posVerbs"> Highlight Verbs</label>
            <label class="switch-item"><input type="checkbox" id="sw-posNouns"> Highlight Nouns</label>
            <label class="switch-item"><input type="checkbox" id="sw-posAdjectives"> Highlight Adjectives</label>
          </div>
        ...
        <div class="panel">
          ...
          <div class="filter-table-wrap">
            <table>
              <thead><tr><th>Word / Phrase</th><th style="width:40px"></th></tr></thead>
    ...
    const renderFilters = () => {
      els.filterBody.innerHTML = [...config.filters].sort().map(f => `
        <tr><td>${escapeHTML(f)}</td><td><button class="btn-small btn-del" data-word="${escapeHTML(f)}">×</button></td></tr>
      `).join("");
    };
    ...
    els.refresh.onclick = () => {
      ...
      const tenseCounts = { present: 0, past: 0, future: 0, modal: 0, perfect: 0, unknown: 0 };
      const posCounts = { adverb: 0, verb: 0, noun: 0, adjective: 0 };
      let wordTotal = 0;
      let charTotal = 0;
      let sentenceTotal = 0;
      ...
      prose.forEach(lineModel => {
        ...
        if (config.switches.filterWords) {
          ...
          if (matches.length) {
            ...
            matches.forEach(match => {
              issues.push({ line: lineNumber, type: "Filter", word: match.word, start: match.start, end: match.end });
            });
          }
        }
        ...
        lineModel.sentences.forEach(sentence => {
          ...
          if (tenseCounts[tense.tense] !== undefined) tenseCounts[tense.tense]++;
          ...
          if (config.switches.tenseCheck && tense.tense !== "present") {
            ...
            if (!tenseMatchesByLine.has(lineNumber)) tenseMatchesByLine.set(lineNumber, []);
            tenseMatchesByLine.get(lineNumber).push(match);
            issues.push({ line: lineNumber, type: tense.label, word: tense.reason });
          }

          if (config.switches.sentenceLen) {
            if (sentence.wordCount > 30) issues.push({ line: lineNumber, type: "Long Sentence", word: `(${sentence.wordCount} words)` });
            if (sentence.wordCount > 0 && sentence.wordCount < 5) issues.push({ line: lineNumber, type: "Choppy", word: `(${sentence.wordCount} words)` });
          }
        });
        ...
        lineModel.wordTokens.forEach((token, index) => {
          ...
          if (config.switches.repeats && clean.length > 3) {
            if (lastSeen.has(clean) && wordTotal - lastSeen.get(clean) <= 15) {
              issues.push({ line: lineNumber, type: "Repeat", word: token.text });
            }
      ...
      els.annotated.innerHTML = prose.map(lineModel => {
        ...
        const tenseMatches = tenseMatchesByLine.get(lineModel.lineNumber) || [];
        const spans = acceptNonOverlappingSpans([
          ...buildHighlightSpans(lineModel, filterMatches, selectedPOS),
          ...buildTenseSpans(tenseMatches)
        ]);
